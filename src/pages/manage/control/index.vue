  ...
  <v-card>
    <v-card-text>
      <v-data-table-server v-model:options="options" :items-per-page="options.itemsPerPage" :headers="headers"
        ...
        :search="`${search.category},${search.type},${search.name}`" :mobile="$vuetify.display.smAndDown"
        ...
        <template v-slot:top>
          <v-row>
            ...
            <v-col>
              <v-select hide-details v-model="search.status" class="pa-2" label="课程状态" :items="status" item-title="name"
                item-value="id" clearable></v-select>
            </v-col>
            <v-col style="text-align: end;">
              <!-- <v-btn style="margin-top: 19px; margin-right: 10px;" class="pa-2" :color="'primary'">批量删除</v-btn> -->
              <v-btn style="margin-top: 19px;" class="pa-2" :color="'primary'" @click="editItem()">新增课程</v-btn>
            </v-col>
          </v-row>
        </template>
        <template v-slot:item.thumbnail="{ item }">
          <v-img :src="useFileUri(item.thumbnail)" :width="80" :height="45" cover>
...
<script setup lang="ts">
  ...
  const save = async () => {
    ...
    try {
      if (coverFile.value) {
        const coverConfig = await FileApi.upload(coverFile.value, 'live/course/cover', true)
        editedItem.value.thumbnail = coverConfig.url
      }
  ...
  const loadItems = async ({
    ...
    const res = await courseApi.getAllCourseTypesByClassfyId({
      types: search.value.type,
      categories: search.value.category,
      page: page,
      size: itemsPerPage,
      name: search.value.name,
      /* status: search.value.status === '审核已通过' ? 1 : 0, */
    })
    serverItems.value = res.records.map((item, index) => {
      return {
        ...item,
